...
def _parse_rank(item) -> Optional[int]:
    ...
    rank_el = item.css_first('.zg-bdg-text, span.zg-bdg-text')
    if rank_el:
        text = rank_el.text(strip=True).lstrip('#').replace(',', '')
        try:
            return int(text)
        except ValueError:
            pass
...
def _parse_rating_reviews(item) -> tuple[float, int]:
    ...
    if rating_el:
        text = rating_el.text(strip=True)
    ...
    review_el = item.css_first('a[href*="customerReviews"] span, .a-size-small.a-link-normal')
    if review_el:
        text = review_el.text(strip=True).replace(',', '')
        match = re.search(r'[\d]+', text)
        if match:
            try:
